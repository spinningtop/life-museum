    ...
    <section className="relative mx-auto max-w-7xl px-6 py-20">
      <div ref={ref} className={`reveal ${isVisible ? 'is-visible' : ''}`}>
        ...
        <div className="mb-16 text-center">
          <p className="mb-3 font-sans text-sm tracking-[0.3em] text-[#e63946]">GUIDE</p>
          <h2 className="font-serif text-4xl font-bold text-[#f1faee] sm:text-5xl">
            导读：马斯克其人
          </h2>
          <div className="mx-auto mt-6 h-px w-24 bg-gradient-to-r from-transparent via-[#e63946] to-transparent" />
        </div>
        ...
        <div className="grid gap-10 lg:grid-cols-2 lg:gap-16">
          ...
          <div className="space-y-10">
            {narratives.map((item, idx) => (
              <div
                key={item.title}
                className="glass-card p-7 transition-all duration-700 hover:border-[#e63946]/30"
                style={{
              ...
              >
                <h3 className="mb-3 flex items-center gap-2 font-serif text-xl font-semibold text-[#e63946]">
                  <span className="h-4 w-1 rounded-full bg-[#e63946]" />
                  {item.title}
                </h3>
                <p className="font-sans text-sm leading-loose text-[#f1faee]/70">
                  {item.text}
                </p>
              </div>
            ))}
          ...
          <div className="space-y-6">
            {prologueQuotes.map((quote, idx) => (
              <figure
                key={quote.id}
                className="glass-card-strong group relative overflow-hidden p-7 transition-all duration-500 hover:glow-red"
                style={{
                  transitionDelay: `${idx * 150}ms`,
                }}
              >
                <QuoteIcon className="mb-3 h-6 w-6 text-[#e63946]/50 transition-colors group-hover:text-[#e63946]" />
                <blockquote className="font-serif text-base leading-relaxed text-[#f1faee]/90">
                  {quote.quote}
                </blockquote>
                <figcaption className="mt-4 border-l-2 border-[#e63946]/40 pl-3 font-sans text-xs text-[#a8dadc]/60">
