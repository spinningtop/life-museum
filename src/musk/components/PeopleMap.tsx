...
import { people, relationships } from '@/musk/data/people';
import { useScrollReveal } from '@/musk/hooks/useScrollReveal';
import type { Person, Relationship } from '@/musk/data/people';
...
// 关系强度 → 连线样式（颜色 / 粗细 / 透明度）
// high 火星红粗线、medium 月光蓝中线、low 月光白细线
function getLineStyle(intensity: Relationship['intensity']) {
  switch (intensity) {
    case 'high':
      return { stroke: '#e63946', strokeWidth: 2.5, opacity: 0.7 };
    case 'medium':
      return { stroke: '#a8dadc', strokeWidth: 1.8, opacity: 0.5 };
    case 'low':
    default:
      return { stroke: '#f1faee', strokeWidth: 1.2, opacity: 0.3 };
  }
}
    ...
    <section className="relative mx-auto max-w-5xl px-4 py-16 sm:px-6 sm:py-20">
      <div ref={ref} className={`reveal ${isVisible ? 'is-visible' : ''}`}>
        ...
        <div className="glass-card relative overflow-hidden p-3 sm:p-4 md:p-6">
          <div className="flex flex-col gap-6 lg:flex-row">
            ...
            <div className="w-full min-w-0 flex-1 overflow-x-auto">
              ...
              >
                <defs>
                  {/* 深蓝星空背景渐变 */}
                  <radialGradient id="starfieldBg" cx="50%" cy="50%" r="55%">
                    <stop offset="0%" stopColor="#1a1a2e" stopOpacity="0.9" />
                    <stop offset="60%" stopColor="#16213e" stopOpacity="0.7" />
                    <stop offset="100%" stopColor="#0a0a0f" stopOpacity="1" />
                  </radialGradient>
                  {/* 主角光晕渐变 */}
                  <radialGradient id="heroGlow" cx="50%" cy="50%" r="50%">
                    <stop offset="0%" stopColor="#f1faee" stopOpacity="0.5" />
                    <stop offset="100%" stopColor="#f1faee" stopOpacity="0" />
                  </radialGradient>
                  {/* 标签内阴影 */}
                ...
                {/* 星空：固定 seed 星点，部分亮星带十字光芒 */}
                <g>
                  {STARS.map((s, i) => (
                    <g key={`star-${i}`}>
                      <circle cx={s.x} cy={s.y} r={s.r} fill="#f1faee" opacity={s.op} />
                      {s.bright && (
                        <>
                          <line
                            x1={s.x - 3}
                ...
                {/* 圈层辅助线（极淡圆形） */}
                <g stroke="#f1faee" strokeOpacity={0.08} strokeWidth={1} fill="none">
                    ...
                    <g key={`side-${rel.id}`}>
                      ...
                      <g transform={`translate(${labelX} ${labelY})`}>
                        <rect
                          ...
                          height="18"
                          rx="4"
                          fill="rgba(241, 250, 238, 0.15)"
                          stroke="rgba(241, 250, 238, 0.4)"
                          strokeWidth="1"
                ...
                {OTHERS.map((person) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {/* 选中高亮环 */}
                      {isSel && (
                        <circle cx={0} cy={0} r={26} fill="none" stroke="#f1faee" strokeWidth={1.5} opacity={0.8} />
                      )}
                      {/* 姓名标签：节点下方 */}
                      <text
                        x={0}
                ...
                <g transform={`translate(${CENTER.x} ${CENTER.y})`}>
                  ...
                  {/* 脉冲圈 */}
                  <circle cx={0} cy={0} r={28} fill="#f1faee" opacity={0.25}>
          ...
          {/* 图例 */}
          <div className="mt-4 flex flex-wrap items-center gap-x-5 gap-y-2 font-sans text-xs text-[#f1faee]/40">
...
function RelationshipCard({
  ...
  return (
    <div>
      <p className="mb-2 font-sans text-xs tracking-[0.2em] text-[#e63946]">关系详情</p>
      <h3 className="mb-3 font-serif text-2xl font-bold text-[#f1faee]">{rel.type}</h3>
      <div className="mb-3 flex items-center gap-2 font-sans text-sm text-[#a8dadc]">
        <span className="font-medium">{fromName}</span>
        <span className="text-[#f1faee]/40">↔</span>
      ...
      <p className="font-sans text-sm leading-relaxed text-[#f1faee]/75">
        {rel.description}
      </p>
    </div>
  );
...
function PersonCard({ person }: { person: Person }) {
  return (
    <div>
      <p className="mb-2 font-sans text-xs tracking-[0.2em] text-[#e63946]">人物简介</p>
      <div className="mb-3 flex items-center gap-3">
        <span
          className="inline-block h-3 w-3 rounded-full"
          style={{ background: person.color }}
        />
        <h3 className="font-serif text-2xl font-bold text-[#f1faee]">{person.name}</h3>
      </div>
      <p className="mb-3 font-sans text-sm font-medium text-[#a8dadc]">{person.title}</p>
      <p className="font-sans text-sm leading-relaxed text-[#f1faee]/75">{person.intro}</p>
    </div>
  );
}
...
function EmptyHint() {
  return (
    <div className="flex h-full min-h-[180px] flex-col items-center justify-center text-center">
      <p className="font-sans text-sm text-[#f1faee]/40">点击星图中的人物或连线</p>
      <p className="mt-1 font-sans text-xs text-[#f1faee]/25">
        查看人物简介与关系详情
      </p>
    </div>
  );
