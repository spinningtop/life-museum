...
import { dramaticMoments } from '@/jobs/data/dramaticMoments';
import { useScrollReveal } from '@/jobs/hooks/useScrollReveal';
import type { DramaticMoment } from '@/jobs/data/dramaticMoments';
    ...
    <section className="relative mx-auto max-w-6xl px-4 py-16 sm:px-6 sm:py-20">
      <div ref={ref} className={`reveal ${isVisible ? 'is-visible' : ''}`}>
        ...
        <div className="mb-12 text-center sm:mb-16">
          <p className="mb-2 font-sans text-xs tracking-[0.25em] text-[#C0C0C0] sm:mb-3 sm:text-sm sm:tracking-[0.3em]">DRAMATIC</p>
        ...
        <div className="relative">
          {/* 中轴线 */}
          <div className="absolute left-4 top-0 h-full w-px bg-gradient-to-b from-[#C0C0C0] via-[#C0C0C0]/40 to-transparent sm:left-1/2 sm:-translate-x-1/2" />
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{
          backgroundColor: isKey ? moment.color : '#0a0a0a',
      ...
      >
        ...
        <div className="flex flex-wrap items-center gap-2">
          <span
            className="rounded-md px-2 py-0.5 font-serif text-[11px] font-bold text-[#0a0a0a] sm:px-2.5 sm:py-1 sm:text-xs"
            style={{ backgroundColor: moment.color }}
          >
            {moment.date}
          </span>
          <span className="font-sans text-[10px] text-[#E5E7EB]/40 sm:text-xs">第{moment.chapter}章</span>
        </div>

        {/* 标题 */}
        <h3
          className={`mt-2 font-serif font-bold text-[#E5E7EB] sm:mt-3 ${isKey ? 'text-xl sm:text-2xl md:text-3xl' : 'text-lg sm:text-xl'}`}
        >
          {moment.title}
        </h3>

        {/* 地点 */}
        <p className="mt-2 flex items-center gap-1.5 font-sans text-[10px] text-[#E5E7EB]/50 sm:text-xs">
          <MapPin className="h-2.5 w-2.5 shrink-0 sm:h-3 sm:w-3" style={{ color: moment.color }} />
          {moment.location}
        </p>

        {/* 描述 */}
        <p className="mt-2 font-sans text-xs leading-relaxed text-[#E5E7EB]/80 sm:mt-3 sm:text-sm">
          {moment.description}
        </p>

        {/* 详细故事 */}
        <p className="mt-2 border-t border-white/10 pt-2 font-sans text-[11px] leading-relaxed text-[#E5E7EB]/55 sm:mt-3 sm:pt-3 sm:text-xs">
          {moment.detail}
        </p>

        {/* 命运分量（茨威格式决定性瞬间） */}
        <div className="mt-2 border-l-2 pl-2 sm:mt-3 sm:pl-3" style={{ borderColor: moment.color }}>
          <span className="font-sans text-[9px] tracking-wider text-[#C0C0C0]/60 sm:text-[10px]">命运分量</span>
          <p className="mt-1 font-serif text-[11px] italic leading-relaxed text-[#E5E7EB]/70 sm:text-xs">
